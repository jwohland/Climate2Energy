...
for date in "2080-02-14" "2080-02-16" "2080-02-18" "2080-12-01" "2080-12-03" "2080-12-05" 
    ...
    for ens in $(printf "%03d\n" {1..10})
      do
        ...
        input_path_discharge="BSSP370cmip6.0001500.${date}.ens${ens}"
	...
	output_path="/net/xenon/climphys/lbloin/CESM2energy/output/boost/${realization}/"
        ...
        python compute_bias_correction.py ${scenario} ${realization} ${bc_realization} ${input_path} ${input_info} ${year} ${output_path} | tee ../logs/compute_bias_correction_log_$(date +%Y_%m_%d_%H)_${input_info}.txt
