...
for date in "2080-02-14" "2080-02-16" "2080-02-18" "2080-12-01" "2080-12-03" "2080-12-05" 
    ...
    for ens in $(printf "%03d\n" {1..30})
      do
        ...
        python compute_generation.py ${scenario} ${realization} ${bc_realization} ${input_info} Wind | tee ../logs/compute_generation_wind_log_$(date +%Y_%m_%d_%H)_${identifier}${input_info}.txt
        python compute_generation.py ${scenario} ${realization} ${bc_realization} ${input_info} PV | tee ../logs/compute_generation_PV_log_$(date +%Y_%m_%d_%H)_${identifier}${input_info}.txt
        conda deactivate
        ...
        # python compute_hydro.py ${scenario} ${realization} ${bc_realization} ${input_info} | tee ../logs/compute_hydro_log_$(date +%Y_%m_%d_%H)_${identifier}${input_info}.txt
        conda deactivate
      done
    done
done
