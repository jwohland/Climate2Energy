...
for year in {1995..2014}
    ...
    for month in 01 02 03 04 05 06 07 08 09 10 11 12
        ...
        cdo sellonlatbox,-20,55,25,80 -mergetime -chname,100u,U ${input_path_ERA5}/"${year}"/100u_1hr_era5_"${year}""${month}".nc ${output_path}/u_"${year}""${month}".nc
        #same for V
        cdo sellonlatbox,-20,55,25,80 -mergetime -chname,100v,V ${input_path_ERA5}/"${year}"/100v_1hr_era5_"${year}""${month}".nc ${output_path}/v_"${year}""${month}".nc
        ...
        cdo sellonlatbox,-15,50,30,75 -remapbil,../inputs/CESM_atm_grid.txt -selname,s_hub -expr,"s_hub=sqrt(U*U+V*V);" ${output_path}/tmp_"${year}""${month}"_s_hub_uv.nc ${output_path}/tmp_"${year}""${month}"_s_hub.nc
