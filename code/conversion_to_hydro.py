...
def create_historical_inflow():
    ...
    for country in tqdm(country_list, desc="Reading reservoir generation per country"):
        ...
        for year in range(year_0, year_N + 1):
            ...
            df_gen_y["time"] = df_gen_y["MTU"].apply(
                lambda x: dt.datetime.strptime(x[:15], date_format)
            )
            df_gen_y["time_delta"] = (
                df_gen_y.time.shift(-1) - df_gen_y.time
            ).dt.total_seconds() / 3600
            df_gen_y["gen_GWh"] = (
                df_gen_y["Hydro Water Reservoir  - Actual Aggregated [MW]"]
                * df_gen_y["time_delta"]
                / 1000
            )
    ...
    eff = 0.9**0.5
    for country in country_list:
        ds_old["inflow_GWh"].loc[{"country": country}] = (
            ds_old["gen"].loc[{"country": country}] / eff
            + ds_old["delta_V"].loc[{"country": country}]
        )
