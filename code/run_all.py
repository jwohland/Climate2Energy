...
ds_wind = ds_wind.drop("lev") #temporary fix to avoid crash - will be fixed when interpolated (bias correction can't have empty lev)
...
# wind
for i in range(3):
    ds_tmp = ds_CF_wind_countries.isel(turbine=i).squeeze() # TODO: squeeze just not be needed once interpolation is done
