...
def bias_correct(da,var,method,lat,lon):
    """takes a data array da of a chosen variable var, and returns the bias corrected version (using package bias_correction)."""
    #making sure that the reference and model data is available
    if glob.glob(f"../output/{var}_ERA5.nc") == [] or glob.glob(f"../output/hist_{var}.nc") == []:
    ...
    model = xr.open_dataset(f"/../output/hist_{var}.nc").TREFHT.sel(lat=slice(30,75),lon=slice(-15,50))
